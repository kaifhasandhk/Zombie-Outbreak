...
selected_power_idx = 0     # Index for the T toggle
active_power = None        # Currently running power
power_timer = 0            # Remaining duration
laser_targets = []         # For visual rendering
...
def draw_player():
    ...
    if is_first_person:
        ...
        glPushMatrix()
        glTranslatef(0, 0, 75)
        # Using gluSphere from your template
    ...
    # --- 3. THE GUN ---
    apply_color(0.15, 0.15, 0.15) # Gun Metal Grey
    glPushMatrix()
...
def idle():
    ...
    global active_power, power_timer, laser_targets  # Added for special powers
    ...
    # --- 1. Update Particles & Special Power Timers ---
    update_particles(dt)
    ...
    if boss_shockwave_active:
        ...
        if abs(dist_wave - boss_shockwave_radius) < 40:
            # SHIELD PROTECTION from Shockwave
            if active_power != "SHIELD":
                # ALSO PROTECT IF INVISIBLE (optional, but logical for "ghost" mode)
    ...
    for z in zombies[:]:
        ...
        # --- FIXED INVISIBILITY LOGIC ---
        # If invisible, zombies are forced to IDLE and skip ALL movement/attack logic
        if active_power == "INVISIBILITY":
            z[5] = "IDLE"
        else:
            # ONLY RUN THIS IF NOT INVISIBLE
            if is_boss:
        ...
        for b in bullets[:]:
            if math.sqrt((z[0]-b[0])**2 + (z[1]-b[1])**2) < hitbox_size:
                z[3] -= b[5]
                special_charge = min(100, special_charge + (1 if is_boss else 3))
                if b in bullets: bullets.remove(b)
...
def keyboardListener(key, x, y):
    ...
    global selected_power_idx, active_power, power_timer, special_charge
    ...
    if key == b'q':
        if special_charge >= 100 and active_power is None:
            ...
            else: power_timer = 10.0 # Shield and Invis get 10s
        else:
            fire_bullet() # Q still shoots if power isn't ready or active
...
# Trigger selected power (Q)
    if key == b'q':
        if special_charge >= 100 and active_power is None:
            ...
            special_charge = 0
            # Set duration based on selection
            if active_power == "LASER": power_timer = 5.0
            else: power_timer = 10.0 # Shield and Invis get 10s
        else:
            pass
    if key == b'f':
